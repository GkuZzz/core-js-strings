...
function isPalindrome(str) {
  const strLowerCase = str.toLowerCase().replace(/[^a-zа-яё0-9]/g, '');
...
function findLongestWord(sentence) {
  const sentToArr = sentence.split(' ');
  let longestWord = '';
  for (let i = 0; i < sentToArr.length; i += 1) {
    if (sentToArr[i].length > longestWord.length) {
      longestWord = sentToArr[i];
    }
  }
  return longestWord;
}
...
function reverseWords(str) {
  ...
  for (let i = 0; i < strToArr.length; i += 1) {
    newArr.push(strToArr[i].split('').reverse().join(''));
  }
  return newArr.join(' ');
}
